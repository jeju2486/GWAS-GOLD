...
if [ ! -f "$result_dir/${prefix}_kmers.txt" ]; then
    ...
    pyseer --lmm --phenotypes "$phenotype" --kmers "$unitig_result"/survived_unitigs.out.pyseer.gz --similarity "$result_dir"/"${prefix}_phylogeny.tsv" --min-af 0.02 --output-patterns "$result_dir"/kmer_patterns.txt --cpu "$threads" > "$result_dir"/"${prefix}_kmers.txt"
else
